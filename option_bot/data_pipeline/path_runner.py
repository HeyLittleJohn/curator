...
class MetaDataRunner(PathRunner):
    ...
    def generate_path_args(self) -> list[str]:
        ...
        log.info("generating file paths for metadata")
...
class OptionsContractsRunner(PathRunner):
    ...
    def generate_path_args(self, tickers:list[str]) -> list[str]:
        ...
        log.info("generating file paths for options contracts")
        path_args=[]
...
class Uploader:
    def __init__(self, upload_func: Awaitable, cleaning_func: function,expected_args: int, record_size: int):  # , upload_pk_name: str):
        self.clean_data = []
    ...
    async def clean_and_process_data(self, )
